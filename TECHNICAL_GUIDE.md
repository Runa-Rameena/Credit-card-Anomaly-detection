# 🛠 Technical Architecture & Multi-Node Setup Guide

This document provides a comprehensive technical breakdown of the **Real-Time Credit Card Anomaly Detection Pipeline**, detailing the distributed cluster configuration, network environment parameters, multi-node execution flow, and storage schemas.
...
```mermaid
flowchart TD
    subgraph Data_Generation["Data Generation & Messaging"]
    ...
    subgraph Processing_Engine["Distributed Processing Engine"]
        C["Apache Spark Master Node<br/>(Laptop 1 Driver)"]
        W1["Spark Worker Node 1<br/>(Laptop 1 Executors)"]
        W2["Spark Worker Node 2<br/>(Laptop 2 Executors over Network)"]
        
        B -->|Micro-Batch Read Stream| C
        C -->|Distribute Micro-Batches| W1
        C -->|Distribute Micro-Batches| W2
    ...
    subgraph Data_Persistence["Data Persistence & UI"]
        M[("MongoDB Collection<br/>(Operational NoSQL Sink)")]
        S[("Snowflake Data Warehouse<br/>(Analytical Warehouse Sink)")]
        UI["Streamlit Dashboard<br/>(Real-Time Monitoring & Overrides)"]
        
        C -->|Parallel Micro-Batch Write| M
        C -->|Parallel Micro-Batch Write| S
...
## 💻 2. Multi-Laptop Cluster Deployment Guide
...
1. Connect Laptop 2 to the same local network.
...
4. Verify worker connection by opening `http://<MASTER_IP>:8080` in your web browser. Laptop 2 will appear under the **Alive Workers** table.

---

## 🗄 3. Storage Schemas

### 3.1 MongoDB Document Schema (`credit_card_db.transactions`)
```json
{
...
### 3.2 Snowflake Data Warehouse DDL (`TRANSACTIONS`)
```sql
...
To stop all background cluster services cleanly:
```bash
